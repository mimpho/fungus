...
**Estado frontend**: Auth completo (JWT, perfil, editar/eliminar cuenta). i18n ES/CA/EN completo. Admin: ImageGenerator (`/admin/generator`) + AdminGallery (`/admin/gallery`) con toggle Público/Admin. Catálogo: 200 zonas + 202 especies desde API.
**Estado backend**: v5.0. Auth JWT: register/login/refresh/logout/me. Tabla `users` con `role` (`"user"`|`"admin"`). Weather cache + Outbreak Index. Endpoints `/species` con `?lang=`.
