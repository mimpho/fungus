  ...
  useEffect(() => {
    ...
    function initGoogle() {
      ...
      window.google.accounts.id.renderButton(container, {
        ...
        text: 'continue_with',
        theme: 'filled_black',
        width: 400, // Force a width larger than the modal to ensure overlap
      })
    }
        ...
        <div className="flex gap-6 mb-6 border-b border-white/[0.08]">
          {[['login', t.iniciarSesion ?? 'Iniciar sesión'], ['register', t.registrarse ?? 'Registrarse']].map(([key, label]) => (
            <button
              key={key}
              onClick={() => setTab(key)}
              className={`pb-3 text-sm font-medium transition-all border-b-2 -mb-px ${tab === key
        ...
        {GOOGLE_CLIENT_ID && (
          <>
            ...
            {/* Inverted overlay pattern:
                - Google's real button is rendered visibly (required by FedCM — Chrome
                  blocks pointer events on iframes whose ancestor has opacity: 0)
                - Our custom-styled div sits on top with pointer-events-none so clicks
            ...
            <div className="relative w-full rounded-xl overflow-hidden min-h-[48px] flex items-center justify-center">
              <div
                ref={googleBtnRef}
                className="w-full flex justify-center"
                style={{
                  background: 'var(--color-modal)',
                  transform: 'scale(1.4)' // Scale up so its invisible clickable area exceeds our boundaries
                }}
              />
              ...
              >
                ...
                >
                  ...
                  {googleLoading
                    ...
                    : (t.continuarConGoogle ?? 'Continuar con Google')}
                </div>
              </div>
            </div>
          </>
