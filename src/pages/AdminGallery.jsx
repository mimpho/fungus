...
function GalleryCard({ s, onOpen }) {
  ...
  return (
    <button
      onClick={() => onOpen(s)}
      className="group relative rounded-2xl overflow-hidden glass hover-lift text-left flex flex-col cursor-pointer"
...
// onOpen prop: si se provee, se llama con la especie en lugar de navegar al generador.
// Usado por AdminGeneratorHub (v5.4) para abrir SpeciesAdminModal sin cambiar de página.
export default function AdminGallery({ onOpen } = {}) {
  const { t } = useApp()
  ...
  // Si se provee onOpen (hub mode), usarlo; si no, navegar al generador (compat standalone)
  const openInGenerator = (s) => onOpen ? onOpen(s) : navigate(`/admin/generator?especie=${s.id}`)
