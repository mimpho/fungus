...
| `memory/business.md` | Estrategia de negocio y monetización (privado, en `.gitignore`) |
| `CHANGELOG.md` | Historial de versiones |

### Regla: documentar decisiones de producto y negocio

Cuando en una conversación surjan temas relacionados con **monetización, modelo de negocio, features premium, pricing, gamificación, estrategia de producto o roadmap comercial**, Claude debe:

1. Contribuir activamente a la discusión con ideas y análisis
2. Al finalizar, proponer proactivamente documentar lo decidido en `memory/business.md`
3. Si el usuario acepta, actualizar el archivo en la misma sesión

Este archivo es privado (`.gitignore`) y es la fuente de verdad para decisiones de negocio, igual que `memory/decisions.md` lo es para decisiones técnicas.
...
| v5.2 | ✅ Entregado | Generador: gallery click → generador con pre-carga · panel de referencia · modal de guardado en catálogo (`PATCH /species/{id}/images`) |
| v5.3 | ✅ Entregado | Generador: fotos ilimitadas · `CatalogImagesModal` unificado · DnD insertion-style (framer-motion layout) · `POST /images/set-order` · galería dinámica SpeciesModal (1/2/3/4/5+ fotos) |
...
| v5.4 | 🚧 Próximo | Rediseño generador admin: galería-first · modal especie con lightbox · sidebar generador simplificado · eliminar selector/ID/CSV/Nuevo |
