...
-- esp-086 Tuber melanosporum + esp-188 Clathrus archeri
-- Apostrophes escaped as '' for PostgreSQL string literals
-- Generado: 2026-03-14
...
SET extra_data = extra_data || '{
  ...
  "cond_req_es": "Hipogeo ectomicorrizógeno estricto con encinas y robles. Requiere árboles huésped adultos de al menos 5–7 años. No puede cultivarse sin plantación específica de árboles micorrizados.",
  "cond_req_ca": "Hipogeu ectomicorrizogen estricte amb alzines i roures. Requereix arbres hostes adults d''almenys 5–7 anys. No es pot cultivar sense plantació específica d''arbres micorrizats.",
...
SET extra_data = extra_data || '{
  ...
  "cond_req_es": "Especie invasora de origen australiano, cada vez más frecuente en Europa. No requiere árbol huésped. Su olor fétido atrae insectos que dispersan las esporas. No comestible.",
...
SELECT id, scientific_name, family,
  left(extra_data->>'cond_temp_es', 60) AS preview
FROM species WHERE id IN ('esp-086', 'esp-188');
