// ==================== ARTÍCULO: MICORRIZAS ====================
// Contenido del artículo "Micorrizas: la alianza secreta entre hongos y árboles".
// Depende de: ArticleSection, ArticleP, ArticleCallout, ArticleInfographic
// (definidos en ArticleModal.js, cargado antes que este archivo).
// Al final del archivo se registra en ARTICLE_REGISTRY para que ArticleModal
// pueda renderizarlo sin conocer los detalles del contenido.
...
function SvgEctoEndo() {
  ...
  return (
    <svg viewBox="0 0 600 260" xmlns="http://www.w3.org/2000/svg" className="w-full rounded-xl">
      ...
      {[
        [endoOx+cellW+gap, ectoOy+cellH+gap],
        [endoOx+2*(cellW+gap), ectoOy],
      ].map(([cx, cy], i) => {
      ...
      {trees.map((tree, ci) => (
        <g key={ci}>
          <text x={labelW + ci*colW + colW/2} y={24}
                fill="#f4ebe1" fontSize="15" textAnchor="middle" opacity="0.8">{treeEmojis[ci]}</text>
          <text x={labelW + ci*colW + colW/2} y={42}
                fill="#f4ebe1" fontSize="9.5" textAnchor="middle" opacity="0.6" fontWeight="500">{tree}</text>
        </g>
      ))}
      {species.map((sp, ri) => {
...
// ── Registro en ArticleModal ───────────────────────────────────────────────────
// ArticleModal usa ARTICLE_REGISTRY[slug] para renderizar el contenido correcto.
// Añadir una línea aquí = nuevo artículo disponible en la app.
