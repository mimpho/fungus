...
function SearchFilterBar({ value, onChange, onClear, placeholder, onFilterClick, activeFilters = 0, variant = 'full', className = '' }) {
  ...
  // Siempre un único <input> montado — evita remount al cambiar variant (que causaba pérdida de foco).
  // El botón Filtrar solo aparece en 'split'; el input adapta su border-radius.
  const hasFilter = variant === 'split';
  const inputRadius = hasFilter && !focused ? `${R} 0 0 ${R}` : R;

  return (
    <div className={`flex ${className}`} style={{ gap: '2px' }}>
      {/* Input — siempre el mismo nodo DOM */}
      <div className="relative flex-1">
        {/* Lupa izquierda — siempre visible */}
        <span className="absolute left-5 top-1/2 -translate-y-1/2 text-[#f4ebe1]/50 pointer-events-none">
          {IC.search}
        </span>
        ...
        {/* X para limpiar — visible cuando hay texto.
            onMouseDown preventDefault evita que el input pierda el foco antes de que dispare onClick. */}
        {value && (
          <button
      ...
      {/* Botón Filtrar — solo en variant split, se colapsa al enfocar */}
      {hasFilter && (
        <div
          className="overflow-hidden shrink-0"
          style={{
            maxWidth: focused ? 0 : '160px',
            opacity: focused ? 0 : 1,
          ...
          <button
            ...
            <span>Filtrar</span>
            {activeFilters > 0 && (
              <span className="px-1.5 py-0.5 bg-[#887b4b] text-white rounded-full text-[10px] font-bold leading-none">
                {activeFilters}
              </span>
            )}
          </button>
